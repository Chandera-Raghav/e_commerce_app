...
class ApiHelper {
  Future<dynamic> getApi(
      {required String url, Map<String, dynamic>? mBodyParams}) async {
    try {
      var res = await http.post(Uri.parse(url),
          body: mBodyParams != null ? jsonEncode(mBodyParams) : {});
  ...
  // Function<dynamic> getUserList({required String Url, Map<String, dynamic>? mBodyParams}) async {
  //   try{
  ...
  dynamic returnJsonResponse(http.Response response) {
    print(
        "${response.body}, ${response.bodyBytes}, ${response.contentLength}, ${response.statusCode}");
